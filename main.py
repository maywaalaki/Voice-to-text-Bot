...
AI_POLISH_PROMPT = """nadiifi qoraal kaan accurately in its original language.
...
def polish_transcript(text):
    ...
    try:
        ...
        logging.warning("Polish step failed: %s", e)
        return text
...
async def handle_media(client, message):
    ...
    try:
        ...
        try:
            ...
            pass
        polished = await loop.run_in_executor(None, polish_transcript, final_text)
        sent = await send_long_text(client, message.chat.id, polished, message.id, message.from_user.id)
        if sent:
            user_transcriptions.setdefault(message.chat.id, {})[sent.id] = {"text": polished, "origin": message.id}
            if len(polished) > 0:
                try:
                    await client.edit_message_reply_markup(message.chat.id, sent.id, reply_markup=build_action_keyboard(len(polished)))
                except:
                    pass
    except Exception as e:
