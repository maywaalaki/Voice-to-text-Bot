...
async def choose_mode(client, message):
    if await ensure_joined(client, message):
        ...
        await message.reply("How do I send you long transcripts?:", reply_markup=kb)
...
async def lang_command(client, message):
    if await ensure_joined(client, message):
        kb = build_lang_keyboard("file")
        await message.reply("Select the language spoken in your audio or video:", reply_markup=kb)
...
async def handle_media(client, message):
    ...
    if getattr(media, 'file_size', 0) > MAX_UPLOAD_SIZE:
        await message.reply(f"Just send me a file less than {MAX_UPLOAD_MB}MB 😎")
        return
