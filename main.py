...
WEBHOOK_URL_BASE = os.environ.get("WEBHOOK_URL_BASE", "inc-katusha-martigaliyebots-dc419d2a.koyeb.app/")
...
AI_POLISH_PROMPT = """clean this text accurately in its original language.
...
Return only the cleaned text in the original language, formatted according to the rules above. Do not prepend or append any meta commentary or labels.
"""

def polish_transcript(text):
    if not text or not groq_rotator.keys:
        return text
    try:
        cleaned = ask_groq(text, AI_POLISH_PROMPT)
        if cleaned and isinstance(cleaned, str) and cleaned.strip():
            return cleaned.strip()
        return text
    except Exception as e:
        logging.warning(f"Polish step failed: {e}")
        return text
...
def handle_media(message):
    ...
    try:
        ...
        try:
            ...
            pass
        polished = polish_transcript(final_text)
        sent = send_long_text(message.chat.id, polished, message.id, message.from_user.id)
        if sent:
            user_transcriptions.setdefault(message.chat.id, {})[sent.message_id] = {"text": polished, "origin": message.id}
            if len(polished) > 0:
                try:
                    bot.edit_message_reply_markup(message.chat.id, sent.message_id, reply_markup=build_action_keyboard(len(polished)))
                except:
                    pass
    except Exception:
